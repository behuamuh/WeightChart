...
export default function(w, lang) {
  ...
  titleChart.innerText =
    Object.keys(w).length == 0 ? TITLE_CHART[0] : TITLE_CHART[1];
  ...
  function draw() {
    ...
    const drawKeys = keys.filter((key, index) => index % compress === 0);
    ...
    const dynamic =
      weigths[drawKeys[0]] - weigths[drawKeys[drawKeys.length - 1]];
    ...
    result.innerText = `${RESULT_CHART[0]}${new Date(
      drawKeys[0]
    ).toLocaleDateString()} - ${new Date(
      drawKeys[drawKeys.length - 1]
    ).toLocaleDateString()}
  ${RESULT_CHART[1]} ${
      dynamic >= 0 ? RESULT_CHART[2] : RESULT_CHART[3]
    } ${Math.abs(dynamic)} ${RESULT_CHART[4]}.
  ${dynamic > 0 ? RESULT_CHART[5] : RESULT_CHART[6]}`;
  }
